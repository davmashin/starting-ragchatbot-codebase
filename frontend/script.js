...
async function startNewChat() {
    // Clear current session on backend if one exists
    if (currentSessionId) {
        try {
...
function applyTheme(theme) {
    ...
    if (themeToggle) {
        const currentLabel = theme === 'dark' ? 'Switch to light theme' : 'Switch to dark theme';
        themeToggle.setAttribute('aria-label', currentLabel);
    }
}